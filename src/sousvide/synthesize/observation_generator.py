...
from sousvide.synthesize.image_modality import (
    VisualModality,get_aligned_stack_files,is_event_cloud_modality,
    prepare_rollout_images,uses_modality_observation_folder,
...
def generate_observation_data(cohort:str,roster:list[str],
                              ...
                              event_cloud_options:dict|None=None) -> None:
    """
        ...
        for student in roster:
            ...
            for course in courses:
                ...
                for idx_ds,(traj_data_path,imgs_data_path) in enumerate(stack_paths):
                    ...
                    if is_event_cloud_modality(image_modality):
                        for rollout in imgs_ds:
                            if (rollout.get("event_cloud_config")
                                    != expected_cloud_config):
                                raise ValueError(
                                    "Event cloud configuration in rollout "
                                    f"artifact '{imgs_data_path}' does not "
                                    "match event_cloud_options.")
...
def generate_observations(pilot:Pilot,
    ...
    for traj_data,imgs_data in zip(traj_set,imgs_set):
        ...
        if is_event_cloud_modality(image_modality):
            dimensions = tuple(Rgbs.shape[1:])
        else:
            dimensions = tuple(Rgbs.shape[1:3])
        if expected_dimensions is None:
            expected_dimensions = dimensions
        elif expected_dimensions != dimensions:
            raise ValueError(
                "Visual input dimensions differ within the rollout stack.")

        if (image_modality == "rgb" and dimensions == (224,224)):
        ...
        observations = {
            ...
            "event_cloud_config":imgs_data.get("event_cloud_config",{}),
        }
...
def save_observations(cohort_name:str,course_name:str,
                      ...
                      image_modality:VisualModality="rgb") -> None:
    ...
    for topic_name in syllabus:
        ...
        cloud_configs = [
            item.get("event_cloud_config",{}) for item in Observations]
        if any(config != cloud_configs[0] for config in cloud_configs[1:]):
            raise ValueError(
                "Event cloud configuration differs within an observation stack.")
        data = {
            ...
            "event_cloud_config":cloud_configs[0],
        }
